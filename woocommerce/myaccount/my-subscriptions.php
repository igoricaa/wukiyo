...
<div class="woocommerce_account_subscriptions">
	...
	<div class="my-orders-wrapper">
		...
		<div class="my-order">
			<p><span>Subscription: </span><?php echo esc_html( sprintf( _x( '#%s', 'hash before order number', 'woocommerce-subscriptions' ), $subscription -> get_order_number() ) ); ?></p>
			<p><span>Date: </span><time datetime="<?php echo esc_attr( $subscription -> get_date_created()->date( 'c' ) ); ?>"><?php echo esc_html( wc_format_datetime( $subscription -> get_date_created() ) ); ?></time></p>
			...
			<a class="my-acc-action" href="<?php echo esc_url( $subscription->get_view_order_url() ) ?>" class="woocommerce-button button view"><?php echo esc_html_x( 'View', 'view a subscription', 'woocommerce-subscriptions' ); ?></a>	
		</div>
		<?php endforeach; ?>
	</div>
	...
	<?php else : ?>
		<p class="no_subscriptions woocommerce-message woocommerce-message--info woocommerce-Message woocommerce-Message--info woocommerce-info">
			<?php if ( 1 < $current_page ) :
				...
				?>
				<br>
				<a class="browse-products woocommerce-Button button" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
