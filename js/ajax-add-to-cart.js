function getCouponValue() {
    let value = parseInt($('.cart-discount .woocommerce-Price-amount').text().substring(1));
    if (value) {
        return value;
    } else {
        return 0;
...
(function($) {
    ...
    $(document).on('click', '.single-product-add-to-cart-button, .add-ajax', function(e) {
        e.preventDefault();

        let $buttonClicked = $(this),
        currentQuantity = parseInt($buttonClicked.attr('data-product-current-quantity'));
        ...
        addToCartFromSingleProductPage(productId, quantityToAdd, currentQuantity, $buttonClicked);
    });

    $(document).on('click', '.archive-single-add-to-cart-button, .add-ajax', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                if (currentQuantity == 0) {
                    ...
                    $('.remove-final-archive[data-productid="' + productId +'"]').attr('data-cart-item-key', cartItemKey);
                    $('.add-final-archive[data-productid="' + productId +'"]').attr('data-cart-item-key', cartItemKey);
                }
    ...
    $(document).on('click', '.remove-final, .add-final', function(e) {
        ...
        const productId = $buttonClicked.attr('data-productid'),
            ...
            cartItemKey = productType === 'subscription' ? $('.id-' + productId +'.subscription').attr('id') : $('.id-' + productId + '.one-time').attr('id');
        let currentQuantityClass = $('.product-quantity-' + productId + '.' + productType),
        ...
        if (operator == 'add' && currentQuantity == 0) {
            addToCartFromCheckout(productId, updatedQuantity, currentQuantity, $buttonClicked);
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                let totalParsed = parseFloat($('.cart-total-value').text().substring(1).replace(',', '')),
                    subtotalParsed = parseFloat($('.subtotal').text().substring(1).replace(',', ''));
                const productPrice = parseInt($('#product-price-' + productId + '.' + productType).val()),
    ...
    $(document).on('click', '.remove-final-single-product, .add-final-single-product', function(e) {
        e.preventDefault();

        let $buttonClicked = $(this),
            productInfoClass = isSubscription ? $('#subscription-product-info') : $('#one-time-product-info'),
        ...
        $('.cart-amount').addClass('blue');
        setTimeout(() => {
            $('.add-final-single-product, .remove-final-single-product').removeClass('disabled'); 
            $('.cart-amount').removeClass('blue');
        }, 1000);

        if (cartItemKey.length > 0) {
            updateCartFromSingleProductPage(productId, cartItemKey, updatedQuantity, cartQuantity, productInfoClass);
        } else if (updatedQuantity > 0) {
            addToCartFromSingleProductPage(productId, updatedQuantity, updatedQuantity, $buttonClicked);
        }
    });

    $(document).on('click', '.remove-final-archive, .add-final-archive', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {              
                if (response) {
                    ...
                    productInfoClass.attr('data-current-quantity', updatedQuantity).attr('data-quantity-to-display', updatedQuantity);

                    $('.single-add-to-cart-button').attr('data-product-current-quantity', updatedQuantity);
    ...
    function updateCartFromSingleProductPage(productId, cartItemKey, updatedQuantity, cartQuantity, productInfoClass) {
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: wc_add_to_cart_params.ajax_url,
            ...
            success: function(response) {              
                if (response) {
                    ...
                    $addToCartButton.attr('data-product-current-quantity', updatedQuantity);
    ...
    function addToCartFromCheckout(productId, quantityToAdd, currentQuantity, $buttonClicked) {
        ...
        $.ajax({
            ...
            success: function(response) {
                $('.product-quantity-' + productId + '.' + productType).text(quantityToAdd);
    ...
    function addToCartFromSingleProductPage(productId, quantityToAdd, currentQuantity, $buttonClicked) {
        let subscriptionData = isSubscription ? {
            'subscribe-to-action-input': 'yes',
            ['convert_to_sub_dropdown' + productId]: '1_month',
            ['convert_to_sub_' + productId]: '1_month',
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                productInfoClass.attr('data-current-quantity', quantityToAdd).attr('data-quantity-to-display', quantityToAdd).attr('data-is-disabled', 0);
                $('.single-product-add-to-cart-button').text('CHECKOUT').addClass('added').attr('data-product-current-quantity', quantityToAdd);
    ...
    function updateShippingMethods(subtotalParsed, discount) {
        ...
        if ((subtotalParsed - discount) >= 199 && freeShippingInput.parent().hasClass('hidden')) {
            ...
            shippingCost.text('$0');
        } else if ((subtotalParsed - discount) < 199 && !freeShippingInput.parent().hasClass('hidden')){
